...
function render (geometry, data) {
  ...
  // Create HexaMap
  function DisplayHexa(nameMap){

    nameMap.selectAll('path')
      .data(hexbin(coord))
      .enter().append('path')
      .attr('transform', d => { 
  ...
  function ResetHexamap(){
    ...
    if(d3.select("input.switch").property("checked")){
      displayMap(hexagon, 1);
    }
    d3.selectAll('input.switchAnim').property('checked', false);
  }
